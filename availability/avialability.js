...
(() => {
    ...
    if (currentLatLng.lat && currentLatLng.lng) {
      ...
      (async () => {
        for (let [k, v] of Object.entries(baseLocations)) {
          ...
          if (
            nearestPosition.loc == "" ||
            parseFloat(distance) < parseFloat(nearestPosition.miles)
          ) {
        ...
        localStorage.setItem(
          "nearestLocation",
          JSON.stringify(nearestPosition)
        );
      })();
    }
  }
  (() => {
    ...
    script.src =
      "https://maps.googleapis.com/maps/api/js?key={{ custom_values.gmap_api_key }}&libraries=places&callback=initMap";
    script.async = true;
  ...
  function discountEventSetup() {
    const discountElemtfound = document.querySelectorAll(
      ".date1.active, .date2.active, .date3.active"
    );
  ...
  let currentDate =
    localStorage.getItem(weddingDatekey) ||
    currentCustomer[weddingDatekey] ||
    "";
